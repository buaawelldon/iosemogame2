...
- (void)viewDidLoad {
    ...
    [self fillobj];
    explode=[UIImage imageNamed:@"pow"];
    [self.explodeeffect setImage:explode];
    self.explodeeffect.frame = CGRectMake(tempposx-35, tempposy, 100, 100);
...
- (void) findHighest {
    ...
    float emo[7] = {angryInt,disgustInt,fearInt,happyInt,neutralInt,sadInt,surpriseInt};
    ...
    NSString *teststr=[NSString stringWithFormat:@"Score: %d",ind];
...
- (NSTimer*)createTimer2 {
    ...
    // create timer on run loop
    return [NSTimer scheduledTimerWithTimeInterval:0.8 target:self selector:@selector(timerHit:) userInfo:nil repeats:YES];
}
...
-(void)showobj{
    ...
    if (explodelast==0)
    {
        ...
        self.explodeeffect.frame = CGRectMake(tempposx-35, 450, 100, 100);
    }
